...
namespace XDPaint.Core.PaintObject.Base
{
    public abstract class BasePaintObject : BasePaintObjectRenderer
    {
        ...
        protected abstract void CalculatePaintPosition(Vector3 position, Vector2? uv = null, bool usePostPaint = true, Triangle hitTriangle = null);
        ...
        public void OnMouseHover(Vector3 position, Triangle triangle = null)
        {
            ...
            if (!IsPainting)
            {
                if (triangle != null)
                {
                    CalculatePaintPosition(position, triangle.UVHit, false, triangle);
        ...
        public void OnMouseDown(Vector3 position, float pressure = 1f, Triangle triangle = null)
        {
            ...
            if (!IsPainting && PaintPosition == null)
            {
                if (triangle != null)
                {
                    CalculatePaintPosition(position, triangle.UVHit, true, triangle);
                    LocalPosition = triangle.Hit;
                }
                else
                {
        ...
        public void FinishPainting()
        {
            if (IsPainting)
            {
                ...
                IsPainting = false;
                if (IsPaintingDone && Tool.ProcessingFinished)
                {
                    SaveUndoTexture();
                }
                lineData.Clear();
