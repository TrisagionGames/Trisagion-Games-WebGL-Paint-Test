...
namespace XDPaint.Demo
{
	public class Demo : MonoBehaviour
	{
		...
		private void OpenBucketPanel(float xPosition)
		{
			colorPalette.gameObject.SetActive(false);
			...
			blurPanel.gameObject.SetActive(false);
			gaussianBlurPanel.gameObject.SetActive(false);
			var bucketPanelTransform = bucketPanel.transform;
			bucketPanelTransform.position = new Vector3(xPosition, bucketPanelTransform.position.y, bucketPanelTransform.position.z);
		}
		...
		private void ResetPlates(BaseEventData data)
		{
			if (colorPalette.gameObject.activeInHierarchy || brushesPanel.gameObject.activeInHierarchy || 
			    bucketPanel.gameObject.activeInHierarchy || blurPanel.gameObject.activeInHierarchy || 
			    gaussianBlurPanel.gameObject.activeInHierarchy)
			{
				colorPalette.gameObject.SetActive(false);
				brushesPanel.gameObject.SetActive(false);
				bucketPanel.gameObject.SetActive(false);
